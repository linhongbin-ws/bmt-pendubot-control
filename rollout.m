...
function [x y L latent] = rollout(start, policy, H, plant, cost, j)
%% Code
...
% % lower down the desity of sampling data
% choosing_index = cost.sampling_to_meausre_ratio:cost.sampling_to_meausre_ratio:H;
% x = x(choosing_index,:); y = y(choosing_index,:);
% latent = latent([choosing_index H+1], :); 
% L = L(:, choosing_index);
